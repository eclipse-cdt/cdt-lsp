...
public class SaveActionsConfigurationArea extends ConfigurationArea {
	...
	public SaveActionsConfigurationArea(Composite parent, EditorMetadata metadata, boolean isProjectScope) {
		...
		final SelectionAdapter allListener = new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				formatEdited.setSelection(!formatAll.getSelection());
			}
		};
		this.formatAll.addSelectionListener(allListener);

		final SelectionAdapter editedListener = new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				formatAll.setSelection(!formatEdited.getSelection());
			}
		};
		this.formatEdited.addSelectionListener(editedListener);
	}
